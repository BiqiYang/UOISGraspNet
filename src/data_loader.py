...
class Tabletop_Object_Dataset(Dataset):
    ...
    def __init__(self, base_dir, train_or_test, config):
        ...
        self.scene_dirs = sorted(glob.glob(self.base_dir + '/' + self.train_or_test + '/' + '*/'))
    ...
    def __getitem__(self, idx):
        ...
        # RGB image
        rgb_img_filename = scene_dir + "rgb_{}.jpeg".format(str(view_num).zfill(5))
        rgb_img = cv2.cvtColor(cv2.imread(rgb_img_filename), cv2.COLOR_BGR2RGB)
        rgb_img = self.process_rgb(rgb_img)

        # Depth image
        depth_img_filename = scene_dir + "depth_{}.png".format(str(view_num).zfill(5))
        ...
        # Labels
        foreground_labels_filename = scene_dir + "segmentation_{}.png".format(str(view_num).zfill(5))
...
base_dir = '/research/d6/gds/bqyang/object_localization_network/data/TOD'
